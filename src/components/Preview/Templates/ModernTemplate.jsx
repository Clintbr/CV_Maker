                        ...
                        <div style={{ width: '580px' }}>
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {data.personal.address && (
                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm">
                                        <Icon name="phone" size={14} color={primaryColor}/>
                                        <span className="text-xs text-gray-300">{data.personal.address} {data.personal.zipCode? ',': ''} {data.personal.zipCode}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                                ...
                                <div className="space-y-3">
                                    {data.skills.map((skill) => (
                                        <div key={skill.id} className="space-y-1">
                                            <div
                                                className="flex justify-between text-xs mb-0.5 uppercase text-gray-400">
                                                <span>{skill.name}</span>
                                                <span>{skill.level}%</span>
                                            </div>
                                            <div className="h-1 bg-gray-700 rounded-full overflow-hidden">
                                                <div className="h-full rounded-full"
                                                     style={{background: primaryColor, width: `${skill.level}%`}}></div>
                                            </div>
                                    ...
                                    {data.workExperience.map((job) => (
                                        <div key={job.id} className="relative p-4 rounded-xl bg-white/5 border border-white/10">
                                            ...
                                            <div className="flex justify-between items-start mb-1">
                                                <h3 className="text-sm font-bold text-white">{job.position}</h3>
                                                <span className="text-xs font-mono text-gray-500">{job.startDate} {job.endDate? '-' : 'heute'} {job.endDate}</span>
                                            </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                    {data.projects.map((proj) => (
                                        <div key={proj.id} className="p-4 rounded-xl bg-white/5 border border-white/10">
                                            <h3 className="text-xs font-bold text-white mb-1 uppercase">{proj.title}</h3>
                                ...
                                <div className="space-y-2">
                                    {data.education.map((edu) => (
                                        <div key={edu.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
                                            ...
                                            <span className="text-xs font-mono text-gray-500">{edu.startDate} {edu.endDate? '-' : ''} {edu.endDate}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
